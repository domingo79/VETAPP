...
def show():
    ...
    if not animali:
        empty_state("🐾", "Nessun animale ancora", "Aggiungi il tuo primo animale cliccando il pulsante sopra.")
        return
...
def _card_animale(animale: dict, owner_id: str):
    ...
    with st.expander(f"{icona_specie(specie)} **{animale['nome']}** — {specie} · {animale.get('razza','')}", expanded=False):
        ...
        with col_info:
            st.markdown(f"**Data di nascita:** {format_data(animale.get('data_nascita'))}")
        ...
        if st.session_state.get(f"confirm_del_{animale['id']}"):
            st.warning(f"Sei sicuro di voler eliminare **{animale['nome']}**? Questa operazione è irreversibile.")
            c1, c2 = st.columns(2)
            with c1:
...
def _form_animale(owner_id: str):
    ...
    with st.form("form_animale"):
        ...
        with col1:
            nome = st.text_input("Nome *", value=editing.get("nome", "") if editing else "")
            data_nascita = st.date_input(
                "Data di nascita",
                value=date.fromisoformat(editing["data_nascita"]) if editing and editing.get("data_nascita") else date(2020, 1, 1),
        ...
        if specie == "Gatto":
            ...
            with col6:
                scuderia = st.text_input("Scuderia", value=editing.get("scuderia", "") if editing else "")
